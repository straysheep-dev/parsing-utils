...
class cVBAProject(cPluginParent):
    ...
    def Analyze(self):
        ...
        if decoded[0:4] == [29, 0, 0, 0]:
            ...
            if sys.version_info[0] == 2:
                salt = ''.join([chr(byte) for byte in data[0:4]])
            else:
                salt = bytes(data[0:4])
            for password in GetDictionary(filename):
                try:
                    calculatedsha1 = hashlib.sha1(password.encode(encoding='utf-8', errors='ignore') + salt).hexdigest()
